...
class OrdersDao(
    ...
    def get_order_items_details(self, order_item_id):
        try:
            if not isinstance(order_item_id, int):
                raise ValueError(
                    f"Expected 'order_item_id' to be int, got {type(order_item_id)}"
                )
            ...
            logger.debug(
                f"✅ Retrieved {len(line_details)} meta fields for order_item_id {order_item_id}"
            )
            return line_details

        except Exception as e:
            logger.error(
                f"🚨 Error fetching metadata for order_item_id {order_item_id}: {e}"
            )
            raise
    ...
    def insert_order_item_meta(self, order_item_id, meta_key, meta_value):
        try:
            if not all(
                [
                    isinstance(order_item_id, int),
                    isinstance(meta_key, str),
                    isinstance(meta_value, (str, int)),
                ]
            ):
                raise ValueError(
                    "Invalid input types for inserting order item metadata."
                )
            ...
            logger.info(
                f"✅ Inserted meta_key '{meta_key}' for order_item_id {order_item_id}"
            )

        except Exception as e:
            logger.error(
                f"🚨 Error inserting metadata for order_item_id {order_item_id}: {e}"
            )
            raise
    ...
    def update_order_item_meta(self, order_item_id, meta_key, new_value):
        try:
            if not all(
                ...
                raise ValueError(
                    "Invalid input types for updating order item metadata."
                )
            ...
            logger.debug(
                f"🛠 Updating meta_key '{meta_key}' with new value: {new_value}"
            )
            self.db_helper.execute_sql(sql, params)
            logger.info(
                f"✅ Updated meta_key '{meta_key}' for order_item_id {order_item_id}"
            )

        except Exception as e:
            logger.error(
                f"🚨 Error updating metadata for order_item_id {order_item_id}: {e}"
            )
            raise
    ...
    def delete_order_item_meta(self, order_item_id, meta_key):
        try:
            if not isinstance(order_item_id, int) or not isinstance(meta_key, str):
                raise ValueError(
                    "Expected 'order_item_id' as int and 'meta_key' as str."
                )
            ...
            logger.debug(
                f"🗑️ Deleting meta_key '{meta_key}' for order_item_id {order_item_id}"
            )
            self.db_helper.execute_sql(sql, params)
            logger.info(
                f"✅ Deleted meta_key '{meta_key}' for order_item_id {order_item_id}"
            )

        except Exception as e:
            logger.error(
                f"🚨 Error deleting metadata for order_item_id {order_item_id}: {e}"
            )
            raise
