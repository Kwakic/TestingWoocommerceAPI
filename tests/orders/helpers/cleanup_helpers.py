...
#     """
#     Set a default APIClient instance for module-level convenience helpers.
#
#     Use case:
#       - Call this once (e.g., from large_conf.py) to expose the legacy module-level cleanup_* helpers.
#
#     Example (large_conf.py):
...
#
#     🔧 Usage:
#         Called from large_conf.py fixture to clean up any WooCommerce resources
...
#         - Avoid creating a APIClient instance at module import time (so tests / CI can control instantiation
#         via fixtures).
#         - Provide helpers to create pre-bound cleanup callables from a APIClient instance (factory-style).
...
#             # callers often pass a bound APIClient.delete_it.py method which accepts an endpoint string and params
#             delete_method(f"{resource_type}/{resource_id}", params={"force": True})
...
# # error if the default APIClient is not set via `set_default_request_utility(...)`.
# #
# # This prevents accidental network calls during import time while allowing a small migration path:
# #   1) In large_conf.py, after creating your request_utility fixture, call:
# #         from EcommerceAPI.src.helpers.cleanup_helpers import set_default_request_utility
# #         set_default_request_utility(request_utility)
...
#             "set the default client with set_default_request_utility(request_utility) in your large_conf.py."
#         )
...
# #     If you want to use cleanup_helpers.py independently of large_conf.py
#
#
