...
def test_customer_deletion_removes_resource(all_resources, customer_helper, customers_dao, create_valid_customer):
    """
    Verify that deleting a customer removes the resource completely.
    ...
    Test flow:
        ...
        3. Verify API no longer returns the customer (404)
        4. Verify the customer record is removed from the database
    ...
    # Step 4 — Verify customer is removed from database
...
def test_deleted_customer_not_in_created_after_filter(customer_helper, customers_dao, all_resources,
    ...
    """
    Verify that a deleted customer does not appear in filtered
    ...
    Test flow:
        ...
        4. Ensure the deleted customer is NOT returned
