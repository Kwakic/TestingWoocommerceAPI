...
def test_list_customers_created_within_time_range_with_db_check(
    customer_helper, customers_dao, minute_offset: int, create_valid_customer
):
    """
    ...
    created_at = isoparse(
        created_at_str
    )  # converted version is: datetime.datetime(2025, 7, 2, 19, 48, 33)
    ...
    assert (
        filtered_customers
    ), "❌ No customers returned from GET /customers with date filters"
    logger.info(
        f"✅ Assertion passed: Customers returned from filtered GET: count={len(filtered_customers)}"
    )

    # Step 4 — Extract the expected customers from dataset
    customer_model = assert_single_customer_by_email(filtered_customers, customer_email)

    # Step 5 — Validate customers identity
    assert_customer_identity(customer_model, customer_id, customer_email)
...
def test_customer_should_not_returned_when_filtered_outside_creation_time(
    ...
    logger.info(
        f"✅ Customer ID {customer_id} correctly excluded from filtered API list"
    )
