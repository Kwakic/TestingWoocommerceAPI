...
def test_bulk_create_customers(qty, all_resources, create_valid_customer):
    """
...
# ---------------------------
@pytest.mark.tcid03
def test_create_single_customer_with_email_and_password_only(all_resources, create_valid_customer):
    """
    ✅ Test Summary:
    This test creates a new customer using the absolute minimum valid input — only `email` and `password`.

    It validates the following aspects:
    1. ✅ API response structure (schema validation) after POST /customers
    2. ✅ Critical fields (`id`, `email`, etc.) are present and correct
    3. ✅ Customer appears in GET /customers endpoint and matches DB
    4. ✅ DB contains the correct record (via DAO lookup)
    5. ✅ Logging and traceability (via logger + correlation_id where applicable)
    ...
    # Access helper/dao via short attribute on all_resources (ergonomic + clear)
...
def test_create_customer_email_field_validation(all_resources, raw_customer_api, payload, expected_status_code):
    """
    ❌ Negative Test: Validate WooCommerce customer creation with malformed or missing email field. This version
    does NOT use create_customer_for_test(), because we expect 400 responses for invalid payloads.
    ...
    🔁 DO NOT use create_customer_for_test() or create_customer() for negative tests where:
    ...
    Instead, for negative tests like test_create_customer_email_field_validation, follow the same model as your
    test_update_customer_invalid_inputs(). The test_update_customer_invalid_inputs() already follows best practice for
    negative testing: it calls the lower-level requests_utility directly and explicitly validates the structure of an
    expected error response.

    Args:
        all_resources(CustomerResources): Fixture providing customer API helpers and DAO.
        raw_customer_api (Callable): ...........
    ...
    # -------------------------------------------
    response = raw_customer_api.post(entity="customers", payload=payload,
